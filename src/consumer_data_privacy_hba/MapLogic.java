...
public class MapLogic {
	...
	public void findDiff(LinkedHashMap <Integer, SortedSet<FrameData>> map,
			LinkedHashMap <Integer, SortedSet<FrameData>> map1) {
		...
		for(Map.Entry<Integer, SortedSet<FrameData>> entry : map.entrySet() ) {
			...
			SortedSet<FrameData> set2=map1.get(chromosome);
			Iterator<FrameData> i=set.iterator();
			Iterator<FrameData> i2=set2.iterator();
			while (i.hasNext() || i2.hasNext()) {
					FrameData obj=(FrameData)i.next();
				FrameData obj2=(FrameData)i2.next();
				if (someMatch(obj,obj2)== false) {
